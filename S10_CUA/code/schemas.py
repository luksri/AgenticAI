...
class ComputerOutput(BaseModel):
    ...
    Mirrors BrowserOutput but for native desktop apps. `path` is the cascade
    layer that actually ran. `content` is the final text result (e.g., the
    Calculator display value). `layer_trace` records every layer tried with
    its outcome and elapsed time — used by replay and the Planner's failure
    routing to understand why escalation happened.
    """
