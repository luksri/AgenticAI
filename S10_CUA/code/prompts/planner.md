...
Available skills:
  ...
  browser            fetch / interact with a SPECIFIC URL through a
                     ...
                     when needed.
  computer           automate a native desktop GUI application via
                     the cua-driver four-layer cascade (hotkeys →
                     AX+LLM → vision). Use when the task requires
                     controlling a GUI app that is NOT a browser:
                       - arithmetic / unit conversion in Calculator
                       - file or folder operations in Finder
                     ...
                     picks the cheapest layer automatically.
                     Do NOT list USER_QUERY in a computer node's
                     inputs — scope it via metadata.goal instead.
...
Scoping a worker — IMPORTANT:
  ...
  - Computer nodes are scoped by `metadata.bundle_id`/`metadata.app`
    and `metadata.goal`. Same rule: do NOT list USER_QUERY in inputs.
