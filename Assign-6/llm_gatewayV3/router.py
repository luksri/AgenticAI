...
def save_limits_state(router: Router | None, router_pool: RouterPool | None, filepath: str = "rate_limits.json"):
    import json
    data = {}
    if router:
        data["worker"] = {}
        for name, state in router.state.items():
            data["worker"][name] = {
    ...
    if router_pool:
        data["router"] = {}
        for name, state in router_pool.state.items():
            data["router"][name] = {
                "calls_today": state.calls_today,
                "tokens_today": state.tokens_today,
                "day_start": state.day_start,
    ...
    try:
        ...
        print(f"[router] failed to save rate limits state: {e}")


def load_limits_state(router: Router | None, router_pool: RouterPool | None, filepath: str = "rate_limits.json"):
    import os, json
    ...
    try:
        with open(filepath, "r") as f:
            data = json.load(f)
        if router and "worker" in data:
            for name, val in data["worker"].items():
                state = router.state[name]
                state.calls_today = val.get("calls_today", 0)
                state.tokens_today = val.get("tokens_today", 0)
                state.day_start = val.get("day_start", state.day_start)
                state.last_call = val.get("last_call", 0.0)
                state.unavailable_until = val.get("unavailable_until", 0.0)
                state.unavailable_reason = val.get("unavailable_reason", "")
        if router_pool and "router" in data:
            for name, val in data["router"].items():
                state = router_pool.state[name]
                state.calls_today = val.get("calls_today", 0)
                state.tokens_today = val.get("tokens_today", 0)
                state.day_start = val.get("day_start", state.day_start)
                state.last_call = val.get("last_call", 0.0)
        ...
        print(f"[router] failed to load rate limits state: {e}")
