...
async def lifespan(app: FastAPI):
    ...
    from router import load_limits_state
    load_limits_state(app.state.router, app.state.router_pool, str(ROOT / "rate_limits.json"))
    yield
...
@app.middleware("http")
async def save_rate_limits_middleware(request, call_next):
    response = await call_next(request)
    if hasattr(app.state, "router") and hasattr(app.state, "router_pool"):
        from router import save_limits_state
        save_limits_state(app.state.router, app.state.router_pool, str(ROOT / "rate_limits.json"))
    return response
