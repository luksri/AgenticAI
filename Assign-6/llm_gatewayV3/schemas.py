...
class ResponseFormat(BaseModel):
    ...
    @classmethod
    def handle_openai_format(cls, data: Any) -> Any:
        if isinstance(data, dict):
            if "json_schema" in data and isinstance(data["json_schema"], dict):
                js = data["json_schema"]
                if "schema" in js and "schema" not in data:
                    data["schema"] = js["schema"]
                if "name" in js and "name" not in data:
                    data["name"] = js["name"]
                if "strict" in js and "strict" not in data:
                    data["strict"] = js["strict"]
        return data
