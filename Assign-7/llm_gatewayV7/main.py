...
async def feed(req: dict):
    """Webpage crawl ingestion from the Chrome extension."""
    url = req.get("url", "")
    title = req.get("title", "")
    ...
    if len(text_to_embed) > E.MAX_INPUT_CHARS:
        text_to_embed = text_to_embed[:E.MAX_INPUT_CHARS]

    try:
        name, result, attempts, latency = await E.embed_with_failover(
            embedders, text_to_embed, "retrieval_document"
        )
    except Exception as e:
        raise HTTPException(502, f"Failed to compute webpage embedding: {e}")
    ...
    agent_memory_dir = ROOT.parent / "Agent" / "sandbox" / "state"
    agent_memory_file = agent_memory_dir / "memory.json"
    agent_memory_dir.mkdir(parents=True, exist_ok=True)

    items = []
    if agent_memory_file.exists():
        try:
            items = json.loads(agent_memory_file.read_text(encoding="utf-8"))
        except Exception:
            items = []

    words = {w.lower() for w in re.findall(r"\w+", text_to_embed)}
    stopwords = {"the", "is", "a", "an", "of", "to", "and", "or", "in", "on", "for", "at", "with", "by", "from"}
    keywords = list(words - stopwords)[:10]

    item = {
        "id": uuid.uuid4().hex,
        "kind": "fact",
        "keywords": keywords,
        "descriptor": f"Webpage: {title}"[:120],
        "value": {"url": url, "title": title, "content": content},
        "embedding": embedding,
    ...
    items.append(item)
    agent_memory_file.write_text(json.dumps(items, indent=2), encoding="utf-8")

    faiss_path = agent_memory_dir / "index.faiss"
    ids_path = agent_memory_dir / "index_ids.json"
