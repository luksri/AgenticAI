...
def _read_for_index(path: str) -> tuple[str, str]:
    ...
    if path.startswith("art:"):
        return _artifacts.artifacts.get_bytes(path).decode("utf-8", errors="replace"), path
    p = _safe(path)
...
def index_document(path: str, chunk_size: int = 400, overlap: int = 80) -> dict:
    """Chunk a sandbox file or artifact and write each chunk into Memory as a searchable `fact`. Use this when the content must remain retrievable across later turns or runs. Example: index_document("notes/spec.md")."""
...
def search_knowledge(query: str, k: int = 5) -> list[dict]:
    """Vector search over indexed `fact` chunks. Returns up to k ranked chunks with provenance. Example: search_knowledge("authentication flow", 5)."""
    items = _memory.memory.read(query, kinds=["fact"], top_k=k)
    return [
        {
            "id": item.id,
